...
21. M148：物件斜角閃光是 `subject_sheen` composite overlay：光帶只存在於陀螺、車、產品或主體的逐幀追蹤 matte 內。`black_to_color` 模式的第一效果幀必須讓 matte 內主體全黑，斜角 frontier 通過後只恢復主體原始彩色並疊一條高對比材質光；手指、臉、背景、文字與全畫面永遠不得變黑或被掃亮。必須同時有 per-frame matte、track/keyframes、材質 profile、遮擋邊緣檢查與首中尾 QA；不規則物件沒有 polygon／alpha matte 就降級 clean hold。拿光帶當轉場、閃到背景或 matte 吃到手指一律 BLOCKED。
