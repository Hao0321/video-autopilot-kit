...
14. M125：戰鬥陀螺真偽是資料，不是每幕都要唸的裝飾。`battle_matchup` 同為 `official` 時，開場、Tracking、HUD 與發布文案只寫雙方陀螺名稱，禁止「正版／正版內戰／正版王牌戰」及底部名稱重複；只有 `official` 對 `counterfeit` 才清楚標示正版／盜版。BEYBLADE X 對戰倒數口號固定寫「3・2・1 Go Shoot！」（英文標準顯示可寫 `3, 2, 1 Go Shoot!`），「發射」只能當一般動作名詞，禁止寫成倒數口號「3・2・1 發射」。勝負標示固定依 Takara Tomy 第 12 版規章使用 `XTREME FINISH +3`、`OVER FINISH +2`、`BURST FINISH +2`、`SPIN FINISH +1`；必須提供 `battle_result` 與逐格事件證據。整顆進區後仍旋轉回 Battle Zone 就取消出界判定；多種 Finish 同時發生或看不清楚時只能 `DRAW / REPLAY`／「判定待確認」，禁止硬猜。`beyblade_x_rules.py` 與 `shorts_gate.py` S-T／S-U／S-V 會 fail-closed；完整規格讀 `references/beyblade-x-finish-judging.md`。
...
21. M148：物件斜角閃光是 `subject_sheen` composite overlay：光帶只存在於陀螺、車、產品或主體的逐幀追蹤 matte 內。`black_to_color` 的黑態必須保留實拍主體原有凹凸、金屬反射與體積，只將它材質化為近黑，不得用純黑幾何塊把主體壓成貼紙；斜角 frontier 通過後只恢復主體原始彩色並疊一條高對比材質光。手指、臉、背景、文字與全畫面永遠不得變黑或被掃亮。必須同時有 per-frame matte、track/keyframes、材質 profile、遮擋邊緣檢查與首中尾 QA；遮罩像圓洞／多邊形貼紙、吃到手部、漂移或沒有完整外輪廓時一律 fail-closed 成 clean hold。人工驗證 keyframes 已含 eased interpolation，不得再二次平滑造成標籤落後；身分字卡完整名稱優先，自動縮字也不得裁尾字。拿光帶當轉場、閃到背景或 matte 吃到手指一律 BLOCKED。
...
27. M155A：大量素材製作固定走 `asset_workshop.py`。視覺層級固定為「真實專案素材→授權真實 B-roll→寫實合成／預渲染 3D→圖形 overlay→clean hold」；高資訊剪輯不等於卡通化。先讀 registry 缺口，只擴產最低覆蓋的語意角色；Imagegen 預設以 4×4 atlas 產可重組原子圖，再由工坊切圖、透明度／重複／授權／能力標示 QA、總覽審核與登錄。寫實生成物一律標 `illustrative_not_evidence`，不可冒充真地點、真商品、價格或實測；`human_review=pending` 永不進自動選擇。預渲染 3D 必須明寫固定視角，不能冒充 mesh。MrBeast／影視颶風只拆資訊功能、材質、聲畫與節奏原理，禁止照搬品牌資產。完整流程讀 `references/asset-workshop.md`。
28. M156A：素材量不是品質。任何批次先做 4–8 格 taste board，Hao 未核准不得大量生成；被判定醜、錯題材或無用的 batch 必須從工作庫、manifest、preview 與衍生檔完整移除。非寫實插畫只允許旅遊／美食／咖啡，固定走日系生活雜誌、自然材質、低飽和、暖白留白與輕鬆節奏；AI／玩具／商業／科技優先真實實拍、誠實 screencast、授權 B-roll、寫實合成或 clean hold。黑底發光 VFX 預設 Screen／Add、白底暗線預設 Multiply；只有遮擋、Tracking、多層合成或交付透明需求才用 `vfx_keyer.py` 轉 Alpha，且原始 master 永不覆寫。
...
34. M160-LAYOUT：參考圖必須先標記學習角色再進設計 DNA。使用者指定為排版／位置／視覺規劃的圖固定走 `layout_only`，只學構圖、層級、圖片位置與文字位置，禁止其 placeholder 色彩、材質與動態進入美術偏好；指定為喜歡美術的圖走 `art_direction`，才學配色、字體、材質與動態。執行順序固定為「role／畫幅 → 一個構圖骨架 → 最多一個圖片位置修飾 → 文字層級 → 一個美術家族 → 材質與有理由的動態」。鈷藍類比拼貼使用單一照片動勢、黑白碎片、Xerox 網點與軌道線；虹彩字體幾何以黑灰硬分區、巨型粗黑／輪廓字與箭頭建立閱讀路徑，光譜色每幀只可落在物件、主字或幾何圓盤其中一個載體。
35. M160-TRACK：高速度追蹤資訊走 `tracked_graphics.py` 的 `telemetry_callout`：panel、elbow connector、anchor dot 必須共享同一可信 track；速度、價格、比分與數量都必須有來源證據，失追先短 hold 後隱藏，不准漂移。快速視差換鏡走 `parallax_transition.py`，固定分類為 `edit_transition`：必須有兩顆真 shot、兩側逐幀前景 matte、重建／clean background、同方向動勢與一個可指出的 midpoint cut；主體在中點硬交接，禁止 cross-dissolve 雙主體殘影。模糊以背景為主，RGB 色散只在峰值 6–14 幀短暫出現；缺任一前提立即降級 `clean_cut`。
...
python autonomy_standard.py queue-status
...
Editkin 與當次最新 Skill 的低 Token `edit-plan/v4`、78 項能力帳本、可續跑 DAG、模型／推理等級 receipt 與 outcome handoff 見
`references/editorial-intelligence-contract.md`；實際步驟一律由 `workflow_contract.json` 與 `workflow_contract.py` 驗證，不再靠 session 記憶順序。
模型選擇、reasoning effort、bounded Markdown router、JSON 執行真相與跨模型語意複核見
`references/model-and-context-adaptation.md`。
...
- M163：「千萬剪輯」固定由 `ten_million_editorial.py` 同時編譯 MrBeast 資訊工藝與影視颶風電影工藝。前者管理 premise、stakes、狀態、尺度、Tracking／VFX 與 payoff；後者管理鏡頭動機、連續性、聲畫、調色／shot match、轉場與克制。衝突時依真實證據→理解→故事主體→連續聲音→華麗度排序，效果不得壓過內容。名稱是品質研發目標，不是流量保證；只有 24 專案 frozen benchmark、blind holdout、逐鏡效果證據與 Hao 時間碼審片通過，才可從 `DIAGNOSTIC` 升級。完整規格讀 `references/ten-million-editorial-system.md`。
- M164：Hao 不在螢幕前時啟用 `autonomy_standard.py`。系統只可做可逆降級（未證實效果停用、怪轉場退 clean cut、未知 Log 停創意 Look），再完成機器 QA、SHA 去重並排入 `videos/_PUBLISH_HUB/_STATE/hao_review_queue.json`。`AUTO_CANDIDATE` 只代表機器證據完整，不是美感認證；未經 Hao 時間碼審片永遠是 `HAO_REVIEW_REQUIRED`、`publish_allowed=false`，任何流程不得代簽、繞過發佈中樞或公開發布。完整狀態機與擴充規則讀 `references/unattended-autonomy-standard.md`。
- M165：所有題材、Shorts／Reels 與長片的程式化動畫和模板，統一由自研 `composition_runtime.py` 的 `hao.motion-composition/v1` 編譯；陀螺只是一個驗證 fixture，絕不是作用域。`visual_director.write_visual_plan()` 每次強制同時輸出 `current_composition.json` 與 `current_render_graph.json`。整數 frame 是唯一時間真相，HTML／component scene／vector scene／3D／Python 效果只連自研 adapter。應用層的時間軸、元件、動畫、模板、路由、Tracking／Roto 工作流、QA 與發佈契約必須自研；FFmpeg、ffprobe、OpenCV、Pillow、瀏覽器／SVG 等成熟低階引擎與標準可直接採用，不得重造 codec、影像矩陣與瀏覽器。HyperFrames、Remotion 與其他 code-video 框架只能作公開能力 benchmark，禁止安裝成 runtime 依賴、複製其程式碼或讓其格式成為專案真相。任何 wall-clock 動畫、隱藏跨景 cross-dissolve、無 meaning／evidence 的 Tracking／Roto／3D、跳過 lint／inspect／Quality-95／手機審片的 render 都 fail-closed。可參數化 variants 必須共享結構雜湊、每個素材以 SHA-256 失效快取；不得把私人路徑、Hao 素材或機器限定資訊寫進公開結構 cache。完整契約讀 `references/programmatic-motion-runtime.md`。
- M166：所有長片、Shorts、Reels 的濾鏡統一走 `filter_runtime.py`、`knowledge/filter_library.json` 與 `knowledge/filter_materials.json`。濾鏡分成 grade／temporal／transition／subject 四類：grade 只能在圖形字幕前套一個 Visual Master Look；temporal 只作用於單鏡；transition 必須有兩顆真實來源鏡頭、明確剪輯動機與對應證據，否則降級 clean cut；subject 必須有逐幀追蹤且經編輯確認的 matte。撕紙不是一張全螢幕模板，而是兩鏡＋時間 alpha＋經 Hao 核准的 Imagegen 紙纖維材質＋可選網點／單色時間濾鏡＋聲音瞬態的 composite preset。禁止 LUT 疊加、濾鏡污染字幕／Logo、全片硬套動態濾鏡、用濾鏡轉場填每個剪點。新濾鏡先通過 registry validation、synthetic motion test、真素材 gallery、Quality-95 與 Hao 手機審片，才可升級自動路由。完整契約讀 `references/filter-library.md`。
- M167：所有新剪輯與重剪一律使用當下專案唯一最新版，不得回退舊 renderer、舊模板或舊 master。Shorts 當前最低收據為 `shorts-v9-2026.08.21`、`visual_plan.version >= 9`、`hao.motion-composition/v1`、`current_render_graph.status=GREEN`，且 `legacy_master_allowed=false`；任一缺失即 fail-closed，不得輸出或註冊成 `current`。發佈中樞 `PUBLISHED` 套件為不可變歷史，批次重剪必須自動略過；只有 Hao 明確要求更正時才走 correction workflow，禁止以新版批次直接覆寫已發布檔案。每次 build／rebuild 都要在 QA 寫入 runtime release receipt，讓「最新版」可機器驗證，不靠記憶或口頭承諾。
- M168：`rebuild／re-render` 絕不等於 `重剪`。Hao 要求「最新版重剪」時，禁止直接沿用舊 `_plan.py` 的選鏡、順序、時長、Hook、節奏曲線、美術與效果決策；必須重新產生 editorial fingerprint，至少包含 shot source／in／duration／order、首 3 秒 promise、energy curve、layout family、palette、motion/VFX events，並與上一版比較。核心欄位沒有實質差異即 `FAKE_RECUT_BLOCKED`，不得交付或宣稱重剪。HyperFrames／Remotion 類能力只能透過自研 runtime 變成可見的動畫、排版、視差、Tracking／VFX 與節奏結果；只更新 JSON、版本號或重新編碼不算完成。
- M169：自動題材 Overlay 必須有 `semantic_target` 與明確功能契約；純裝飾框線、刻度、HUD、三條彎線或題材皮膚不得因 cold open、payoff 或模板名稱自動疊在真實畫面。`food_overlay` 已由 Hao 於 2026-08-21 人工淘汰，任何舊 plan 重新載入也必須 fail-closed；逐幀檢查發現無語意殘留即退回重建。
- M170：戰鬥陀螺「主體」與「設備」必須分開判斷。盜版／仿製陀螺可以玩、可以做正版 VS 盜版內容，前提是真偽標示誠實且不得靠外觀猜；社群高風險的是盜版戰鬥盤與盜版發射器，公開成片預設只使用／露出 Hao 已確認的正版戰鬥盤與發射器，設備真偽不明則停在 `BATTLE_GEAR_AUTHENTICITY_REQUIRED`。Hao 已刪除的未發布舊片必須由 `publish_hub.py withdraw-content` 寫入撤回帳本，不得被同步器、漏登記修復或 READY 匯入復活；此撤回只針對那些成片，不等於全面禁止盜版陀螺題材。
- M171：任何長片、Shorts、Reels、模板、濾鏡、VFX 或素材工坊在語意上需要圖片、照片式合成、固定 bitmap、texture、matte、plate 或 atlas，而 verified project source、合規授權資產與已核准 Imagegen 資產都不存在時，必須由 `imagegen_asset_gateway.py` 輸出 `IMAGEGEN_REQUIRED`，並由當前 Codex 回合直接使用 OpenAI 內建 Imagegen 生成；不得以漸層、色塊、blob、三條線、泛用 ICON、程序噪點或空泛模板冒充 premium 圖片素材。生成物必須複製進專案、由 `asset_workshop.py` 登錄為 `human_review=pending`、`selectable=false`，並依 M159 自動提供手機 HTTPS 審查；只有 Hao 核准後才可進自動選擇。若缺的是地址、價格、戰績、實測、產品外觀或其他真實證據，禁止生成，改停在 `VERIFIED_SOURCE_REQUIRED`。尚未核准或生成失敗時只能 clean hold／clean cut，不能偷用舊垃圾素材。
- M172：Editkin 是預設剪輯執行器。每次 Build 由 `workflow_contract.py` 建立可續跑 run，固定依序 `get_autopilot_contract` → `start_ai_editing_session` → 對每個相關片段 `prepare_ai_material` → 最多四張一批的 `view_material_keyframes` → 有界 `get_material_context` → `record_material_semantics`；全部素材語意 receipt 齊全後，`resolve_autopilot_inference_route` 可與只讀的外掛能力發現並行，接著由當次最新 Skill／knowledge revision/hash 產生 `hao.video-autopilot.edit-plan/v4`，再依序 `audit_autopilot_plan` → `apply_autopilot_plan` 一次原子提交 → `render_project` → 手機人工審片 → `record_autopilot_outcome`。每段判讀至少引用一張關鍵幀或一個逐字稿 cue；素材分析可按 source hash 有界平行與命中快取，但任何 prerequisite、receipt、來源重驗或人工審片都不可跳過。中斷後只重試失敗且可重入的步驟；apply 狀態不明必須先 reconcile，禁止盲目重套。v4 必須綁定 material／semantic／audit receipts，並攜帶 brief、promise/stakes/payoff、ordered energy beats、packaging hypotheses、字幕與圖形分工、motivated transitions、layered audio、single look、semantic asset order、平台 variants、D2/D7/D28 與 inference receipt；v3／v2／v1 只准讀取／匯入，不得 apply 或關閉 current workflow。產品只能保存穩定 schema、structured commands、immutable receipt 與 learning event，不得把私人 Skill／完整記憶凍結進 installer。Hao 以付費 AI 點數生成且明確授權的素材，只有在逐檔 hash manifest、owner attestation 與社群授權文字齊全時可納入安裝包；第三方權利不明仍 fail-closed。完整契約讀 `references/editorial-intelligence-contract.md` 與 `references/editkin-workflow-execution.md`。
- M173：Markdown 能改善長規則的層級與注意力配置，但不能取代 schema、負例 gate 或品質評測。每次 Build 只把 bounded Markdown router 的 SHA-256 寫入 v4，JSON 是唯一執行真相；Sol／Terra／Luna 與 `none|low|medium|high|xhigh|max` 每一格都需同 provenance benchmark 才能標 measured。未量測模型不得 direct apply，editorial／quality／audit／publish-adjacent 工作強制第二次語意複核；被拒絕只准針對 rejection 修復一次，再失敗交 Hao，禁止無限重試燒 Token。完整規格讀 `references/model-and-context-adaptation.md`。
- M174：手機遙控採「掃一次、永久綁定直到撤銷」而非每次重掃 QR。QR 內只放十分鐘內失效的 bootstrap token；配對後改用逐裝置 256-bit credential，桌面只保存其 SHA-256，停止 Remote、重啟軟體或短暫斷網都不得忘記信任裝置。授權與可達性必須分開：跨網路要走自有穩定 HTTPS relay／origin；暫時 tunnel 或同網路測試不得冒充跨網路永久連線。完整契約讀 `references/editkin-mobile-device-binding.md`。
- M175：所有桌面 dispatch 必須以結構化 command 驗證目前選取、來源類型、軌道相容性、時間範圍與路徑邊界；來源素材唯讀，變更要原子化且可 Undo。按鈕、快捷鍵、拖放與 AI/MCP 必須走同一 command contract；只有 UI 顯示、toast 或 mock handler 不算功能完成，無法證明安全執行時 fail-closed。
- M176：開發伺服器、單元測試或原始碼 render 不得冒充交付版可用。只有在 Hao 明確允許打包後，涉及桌面 runtime、bundled resources、字型、FFmpeg、素材或跨程序 IPC 的功能，必須通過 cold extracted delivered journey：全新目錄啟動、真實使用者輸入、儲存／重開／render、資源與 receipt 驗證；安裝包未授權時只跑開發版與可逆測試，不得浪費 Token 反覆封裝。
- M177：Editkin 的 AI 串接預設使用使用者既有 Codex／Claude Code 訂閱 Session，不要求 API key。介面提供一鍵連線與可複製 starter prompt；AI 負責素材理解與產生受 schema 約束的 edit plan，Editkin 原生執行、預覽、Undo、render 與 receipt。優先 MCP／structured command，Computer Use 只作沒有結構化能力時的有界 fallback。
- M178：匯入後可讓使用者選遊戲、美食、旅遊、Podcast 等 editorial profile 與是否露臉；選擇是節奏、字幕、畫面、音樂、B-roll 與版型的強 prior，必須持久化到 plan／project／receipt，但不能取代逐段素材分析，也不能把模板名稱當成內容證據。使用者未選時才由素材語意建議並要求可見確認。
- M179：Podcast／訪談導演台需把每位人物的可編輯 ROI、逐字稿／speaker cue、聲音活動與視覺活動合併成 shot graph，以 hysteresis 防止鏡頭抖動，支援單人、雙人、split screen 與不露臉 B-roll；追蹤不確定就保留 wide／split fallback，禁止假裝已辨識說話者。多語聲紋、重疊語音與同素材競品盲測未通過前只能標 `UNMEASURED`。
- M180：一支長拍攝檔若含 N 個可獨立發布的完整 promise／payoff，來源檔數不得決定成片數。Build 前先建立 closed-world deliverable map；預期 N 支就必須產生 N 個獨立可編輯專案、N 支 render、N 份 receipt，禁止退化成單一合輯或共用可變輸出。舊 Skill／舊軟體成片只能作 `benchmark_only` 負例，不得當 ground truth；真正 recut 必須重新分析原始素材，且新 editorial fingerprint 對選鏡、順序、節奏、圖形、聲音或效果至少有實質差異，否則 `FAKE_RECUT_BLOCKED`。
- M181：所有可泛化的剪輯、美感、節奏、字幕、字體、版型、調色、音訊、Tracking、轉場、B-roll、包裝、題材路由、演算法與負面案例都可編譯成 Editkin 社群版 Knowledge Pack；但姓名、帳號、Email、私人路徑、密鑰、私人素材原檔、未匿名成效、可回推身分的事件與原始參考圖必須排除。編譯器必須以 closed-world 清冊列出每個來源模組的 include／exclude 與理由，綁定來源 SHA-256，通過 schema、PII／路徑／秘密掃描後才可進產品；Agent 只能按題材以有界分頁讀取所需模組，禁止一次把整包記憶塞進 context。公開匿名知識不得取代當次最新 Skill、真素材分析、人工審片與 immutable receipt。
- M182：記憶與素材數量增加不得拖慢主介面啟動。匿名 Knowledge Pack 全文只准由 Agent／MCP 依題材有界分頁讀取；瀏覽器主 UI 只載入與全文 SHA-256 綁定的小型摘要。大型 preset／模板 registry 與自動剪輯 builder 必須以動態 chunk 在使用時載入，Preview／Inspector／素材預覽仍需可發現且載入失敗要顯示明確錯誤。每次 Build 以固定 main JS／gzip／CSS budget 阻擋回歸，不得調高預算或刪功能來假裝優化；功能新增後必須同時重跑 bundle gate、真實 MCP 分頁與完整編輯測試。
- M183：Editkin 外掛能力不得寫死在 Skill。每次 Build 完成素材語意判讀後，先以 `list_installed_plugins(automationReadyOnly=true)` 只讀精簡能力清單；只對與當前 semantic role／片型相符的候選呼叫 `get_plugin_capability`，確認 requires、avoidWhen、參數、權限與 readiness 後，將獲選動作編譯進同一份 v4 plan，禁止在 plan audit 前先改專案。外掛必須走受 schema 限制的 EditGraph 原子命令並留下 manifest SHA-256／project revision receipt；任何 `BLOCKED`、`MANUAL_ONLY`、`RUNTIME_READY` 但 `automationReady=false` 的能力都不得自動套用。安全 `gpu_effect_graph` 或由 `gpu_effect_module` SDK 編譯出的 graph，只有 resident GPU 畫素 oracle、正式輸出同源序列、零 product-path CPU pixel copy 與 fail-closed 負向 gate 全綠，且作者宣告 assisted／full 時才可列入自動候選；module 還必須是無迴圈、無遞迴、無任意 WGSL／資源存取的單一路徑節點圖。同片有明確動機時最多依序堆疊 4 個 program／16 個 operations，必須逐一保留 manifest-pinned program identity、編譯順序與 receipt，CPU/GPU runtime 不混用，輸出不得改用近似濾鏡偷換效果。外掛只加速已成立的剪輯意圖，不能取代真素材證據、效果動機、Quality-95、render QA 或 Hao 審片；新安裝外掛會由 runtime discovery 自動被學會，不把名稱、商店或私人外掛內容凍結進 Skill。完整流程讀 `references/editkin-plugin-automation.md`。
