> **主責／路由**：基本功已做但仍不抓人時讀本文；其他症狀先回 [craft refs 索引](craft-index.md)。本文主寫情緒、故事、連續性、配樂、聲音、二級調色與包裝的高手判斷層。

# 剪輯 master 級：competent → 高手中的高手（2026 深挖 round3 + 對抗驗證）

> **現行執行契約（2026-08-27）**：本文所有工具操作語句都是 editorial intent，不是 GUI SOP。落地時必須編譯為 `hao.video-autopilot.edit-plan/v4` structured commands，逐 command 綁 evidence／時間窗／參數，再由 `workflow_contract.py` audit → accepted receipt → atomic apply → committed receipt → render → delivery QA → human/outcome receipt。任何本機 media primitive 都必須由 plan 與 receipt 可追溯。
...
**Rule of Six 六準則權重表（一刀好不好的優先序評分）**  〔expert-consensus｜Editkin v4 structured command / ffmpeg 通用 — 這是判斷層，不是操作層。剪每一刀前心裡跑一次 6 項評分。〕
...
**Blink Theory 眨眼即剪接點（用演員/說話者的眨眼找這一刀的位置）**  〔expert-consensus｜Editkin v4 structured command（逐格看眼睛找眨眼格）/ ffmpeg（旁白片用 silencedetect/換氣點當剪接候選）〕
...
**Cut-rate ↔ Blink-rate 對齊（用『每分鐘切幾刀』校準整段情緒密度）**  〔data-backed｜ffmpeg/ffprobe（數 cut 數）/ Editkin v4 structured command（看 timeline 密度）〕
...
**Repeat-Cut 一致性測試（同一刀剪三次落點不同＝節奏沒抓到）**  〔expert-consensus｜Editkin v4 structured command（手動重剪比對）〕
...
- competent→master：解決『rhythm 這項說不清、只能憑感覺』——把它變成可重複的身體動作。對純 ffmpeg/Editkin v4 structured command 螢幕前久坐的剪法是個提醒：交付前站起來『全片不暫停看一遍』，身體卡住的地方就是 rhythm 出問題的地方（比逐格 audit 更快抓到悶點）。

**Eye-Trace 視線接續（讓觀眾的注視焦點跨刀不必跳，否則就是壞剪）**  〔expert-consensus｜Editkin v4 structured command（看前後格焦點位置）/ ffmpeg（截接點前後 frame 比對）〕
...
**L-Cut 預期性反應剪接（切在反應、別切在『誰講完換誰』）**  〔expert-consensus ·常識｜Editkin v4 structured command（分軌錯位）/ ffmpeg（音畫分軌 offset）〕
...
**Dissolve = 情緒位移的量度（hard cut vs 溶接代表念頭轉換的『距離』）**  〔expert-consensus｜Editkin v4 structured command / ffmpeg（xfade 控制溶接時長）〕
...
**Match on Action 對到幀（動作連戲剪接）**  〔expert-consensus ·常識｜通用（Editkin v4 structured command 在 timeline 上逐幀對齊兩 clip 的動作幀；ffmpeg 用 -ss 精確到幀切入點）〕
...
**合法跨越軸線：Buffer Shot / Neutral Shot / 鏡頭內轉向（master 級補救）**  〔expert-consensus｜Editkin v4 structured command 或 ffmpeg：在 timeline 插入 buffer/cutaway clip 當橋〕
...
**Graphic Match / 圖形匹配剪接**  〔expert-consensus｜Editkin v4 structured command/ffmpeg：選兩顆構圖相似 clip 對接，必要時微調 scale/position 對齊主體〕
...
- competent→master：解決『轉場生硬 / 只能靠 Editkin v4 structured command 罐頭轉場特效』。用構圖本身過場=高級感、可壓縮時空、可植入隱喻。是『不用特效的特效』。

**Cutaway 遮接 / 隱形時間壓縮**  〔expert-consensus ·常識｜Editkin v4 structured command/ffmpeg：在主軌剪接縫上方疊 B-roll cutaway 蓋住跳接（=Hao M87 對位機制）〕
...
**Walter Murch《In the Blink of an Eye》Rule of Six（決定一個 cut 好不好的優先順序）**  〔expert-consensus ·常識｜通用心法（套用在任何 cut 點決策；Editkin v4 structured command/ffmpeg 皆適用）〕
...
**Equal-Power（等功率）crossfade 接段，不要 linear — 這是「無縫接歌」的物理核心**  〔data-backed｜ffmpeg: acrossfade=d=2:c1=qsin:c2=qsin（接不同段）；Editkin v4 structured command 內建 crossfade 轉場已等功率〕
...
**Cut on the phrase, not the beat — 在「樂句」邊界剪（4/8 小節），不是在每個 beat 上剪**  〔expert-consensus｜Editkin v4 structured command 手動 beat markers（tap 在句尾下拍）；ffmpeg atrim 按算出的小節秒數切〕
...
**Music as emotional spine — 先鋪音樂（temp track）再剪畫面，讓音樂結構驅動剪輯**  〔expert-consensus ·常識｜通用工序（Editkin v4 structured command/ffmpeg 皆適用）：時間軸先鋪音樂再排畫面〕
...
**Spotting / hit points — 把畫面關鍵事件對到音樂的『擊點』**  〔expert-consensus｜Editkin v4 structured command beat markers + 手動微調剪點到 frame；ffmpeg 用精確秒數對齊 overlay/cut〕
...
**Tension build：riser + 高通濾波掃頻堆張力，drop 才有釋放感（build-and-release）**  〔expert-consensus｜ffmpeg highpass 自動化（highpass=f=… 配 volume 包絡）+ aecho/reverb；Editkin v4 structured command 音效庫 riser + 關鍵幀音量〕
...
**Stop-down：drop 到完全靜默當對比 —— 全片至少 2–3 個歸零點**  〔expert-consensus｜ffmpeg：volume=0 包絡 / atrim 切出靜默段 / aselect；Editkin v4 structured command：音量關鍵幀拉到 0 或分割刪段〕
...
**Anticipation cut（提前剪）+ 不全卡點 —— 別把每一刀都釘死在 beat 上**  〔expert-consensus｜Editkin v4 structured command 手動微調剪點（不全靠 Auto Beat Sync）；ffmpeg 精確秒數錯位剪〕
...
**辨識歌曲結構 + Editkin v4 structured command/手動 beat grid 把段落地圖化**  〔expert-consensus ·常識｜Editkin v4 structured command Auto Beat Sync + 手動 tap markers + snap；ffmpeg/ffprobe RMS/能量掃描找 drop〕
...
- 數值/出處：結構訊號：drop 前有 riser/靜默；chorus 織體最厚低頻最重。Editkin v4 structured command 右鍵 Auto Beat Sync（掃峰值自動標）+ 手動 tap beat grid（不依賴偵測）+ clip 邊緣 snap marker。縮短策略：刪 verse/重複，留 build→drop→outro。
...
**Sound Bridge / J-cut & L-cut（聲音先過場縫合）**  〔expert-consensus｜Editkin v4 structured command（分離音訊+左右錯位拖動）/ ffmpeg（afade + adelay）〕
...
- 數值/出處：J-cut：讓 B 場 ambience 提前 12–24 frames（約 0.4–0.8s @30fps）淡入；對白場景用 L-cut 讓 A 角色尾音壓到 B 畫面再收，不超過一句的尾音。Editkin v4 structured command：該段音訊 detach 後左右錯位，視訊不動。ffmpeg：對 ambience 用 adelay 負偏移(提早起點) + afade=t=in:d=0.6 疊在前一段尾巴。
...
**Designing FOR sound — 留白讓聲音說話（Randy Thom）**  〔expert-consensus｜剪接決策層（Editkin v4 structured command / ffmpeg 皆適）〕
...
**Risers / Impacts / Sub-drops — 重點前的張力工程**  〔data-backed｜ffmpeg / Editkin v4 structured command 疊 SFX 軌（riser + impact + sub）〕
...
**Synchresis — 聲畫焊接 / Added Value（Michel Chion）**  〔expert-consensus ·常識｜Editkin v4 structured command / ffmpeg frame-accurate SFX 對位〕
...
**Foley 分層設計（不只腳步—情緒化物件聲）**  〔expert-consensus｜Editkin v4 structured command / ffmpeg 多層 SFX 軌疊加〕
...
**Qualifier(HSL Key)圈選局部 + 三段微調順序：寬度→高低點→柔邊**  〔expert-consensus｜DaVinci Qualifier；Editkin v4 structured command：用『曲線-HSL』分色相通道近似（只能調單一色相不能複合 key）；ffmpeg：selectivecolor 濾鏡（仿 Photoshop 選取色彩，依 reds/yellows/greens 等色域+飽和純度調 CMYK）或 hue/colorchannelmixer〕
...
- 數值/出處：selectivecolor 範例：ffmpeg -i in.mp4 -vf "selectivecolor=reds=.1 0 0 0:yellows=0 0 -.1 0" out.mp4（reds 加青、yellows 減黃）。Editkin v4 structured command HSL 曲線：選『橙』通道單獨降飽和=只壓膚色不動其他
...
**Shot Matching to Reference：先中性化→對比/飽和/色平衡定序 + 雙片比對 wipe**  〔expert-consensus｜DaVinci Stills Gallery + reference wipe + scopes；Editkin v4 structured command 無 wipe，靠並排匯出比對；ffmpeg：hstack 兩片並排比對、或 blend=difference 看差異〕
...
**Vectorscope 膚色線(I-line / Skin Tone Line)+ 雙 scope 配對法**  〔data-backed｜DaVinci/Editkin v4 structured command 內建示波器；ffmpeg：-vf vectorscope=mode=color3 與 waveform 濾鏡可即時疊在畫面上檢查〕
...
**Node / Layer 思維：serial(串) vs parallel(並) vs Layer Mixer(混合模式)**  〔expert-consensus｜DaVinci node graph；Editkin v4 structured command：用多層『調整圖層』堆疊近似 serial、不同圖層套混合模式近似 Layer Mixer；ffmpeg：filter_complex 的鏈接順序=天然 serial，split+blend=parallel/layer mixer〕
...
**色彩心理 / 母題色(motif color)：用單一顏色貫穿敘事 + 飽和/對比=情緒槓桿**  〔expert-consensus ·常識｜概念層→任何工具皆可執行；建一個 3-5 色的固定 palette 寫進 Editkin v4 structured command/ffmpeg 預設，跨片重用=signature〕
...
- 數值/出處：執行=先在腦中/mood board 定情緒再調，不要機械套公式(Kroll：『follow your gut』)。signature 操作化：固定一組『暗部推某色相+亮部推互補色+某去飽和量』數值，存成 ffmpeg 預設或 Editkin v4 structured command 調色檔，每支片套同一組=觀眾認得出『這是你的片』
...
**Day-for-Night：中間調主導降光+冷藍偏洋紅 + 暗部單獨去飽和 + 保留實用光源**  〔expert-consensus｜DaVinci color wheels + window；Editkin v4 structured command 調色面板；ffmpeg：curves(降midtone)+colorbalance(推冷)+eq(去飽和)+ 對實用光源用 maskedmerge 保留〕
...
**Halation / Glow / Bloom 底片質感：摳亮部→高斯模糊→暖紅染色→screen/lighten 疊回**  〔expert-consensus｜DaVinci(摳亮部 node 放最前+gaussian blur+紅 tint+composite Screen)；Editkin v4 structured command 無原生，靠複製圖層+模糊+變亮混合模式近似；ffmpeg：split+lutyuv摳亮部+gblur+colorbalance染紅+blend=screen/lighten〕
...
**Package-Before-Produce（先包裝後生產 / 25-50 標題 × 3-5 縮圖門檻）**  〔expert-consensus｜紙筆 / Figma / Canva（拍之前，不用 Editkin v4 structured command）〕
...
**Murch 六法則：emotion > continuity（每一刀的決策階層）**  〔expert-consensus｜Editkin v4 structured command / ffmpeg 通用(這是決策框架不是功能)〕
...
**J-cut / L-cut(split edit)— 用聲音「拉著」畫面,讓剪接隱形**  〔expert-consensus ·常識｜Editkin v4 structured command(音訊/視訊軌分離拖移)、ffmpeg(adelay + 偏移 concat)〕
- 做法：J-cut=下一段的「聲音」先進來、畫面後到(audio lead),製造前進感/預期;L-cut=這段聲音延續到下一畫面(audio lag),製造情緒餘韻。實作:在 Editkin v4 structured command 把音訊軌與視訊軌分開,聲音比畫面早 6–12 frame(J)或晚 6–12 frame(L)切。人腦追連續聲音,聲音不斷=剪接無感。
...
**Whip pan match cut — 把剪接藏在運鏡模糊裡(無縫換地點)**  〔expert-consensus｜Editkin v4 structured command(逐 frame 對接點)+ 拍攝端慢快門〕
...
- 數值/出處：重機 motovlog 直式 Shorts 也吃這套:過彎甩頭/甩 GoPro → 接下個路段;Hao 拍時要刻意『多甩一下』留素材。接點對齊技巧:把兩 clip 都放到最模糊 frame,在 Editkin v4 structured command 逐 frame 對。
- competent→master：competent editor 用 Editkin v4 structured command 內建『轉場特效』硬套(觀眾一眼看出是模板)。master 是用「素材本身的運動」當轉場,看起來像一鏡到底——這是 cinematic travel 流派(Sam Kolder 系)最核心的 signatu…

**Speed ramp 速度曲線(非單一變速)— 直線加速、彎道/落點減速**  〔data-backed ·常識｜Editkin v4 structured command『速度→曲線』(velocity curve,非常規變速)〕
- 做法：用『速度曲線(velocity curve)』而非 Editkin v4 structured command 的單一『慢/快』:在一個 clip 內 0.5×→4×→1× 平滑過渡,用 Bezier handle 拉曲線讓加減速有「重量感」。公式化用法:無聊路段/長廊 4–8× 衝過去,抵達點/精彩處 ramp 回 0.5× 落地。拍攝端用高 fps(60/120)才有慢動作餘裕。
- 數值/出處：重機 motovlog:直線路段 4–6× + 引擎聲保留當節奏;旅遊:走進景點長廊 4× → 看到主景 ramp 到 0.5× + 同步 BGM drop。Hao 拍 GoPro 設 60fps 以上留變速空間。
- competent→master：competent editor 整段勻速快轉(像快進帶),觀眾無聊。master 用曲線製造『呼吸感』——加速帶過廢話、減速強調重點,這是把 30 分鐘騎車/旅程濃縮成 3 分鐘還好看的唯一方法。Editkin v4 structured command 多數人只按『2×』,沒碰 velocity …

**美食:overhead flatlay 文法 + 聲音「重置/替換」(ASMR 流派)**  〔data-backed ·常識｜Editkin v4 structured command(音效分層 + 0.3s 快剪)/ ffmpeg(音效 overlay 對齊)〕
...
**Tech 教學:Fireship 流派 — jump cut 去填充詞 + punch-in 變焦保持眼球**  〔expert-consensus ·常識｜Editkin v4 structured command(jump cut + 關鍵格放大動畫 + SFX)/ ffmpeg(zoompan punch-in)〕
...
**Tech 教學:MKBHD 流派 — clean B-roll 解釋抽象概念 + 不過度調色**  〔expert-consensus ·常識｜Editkin v4 structured command / ffmpeg + Hao 既有 caption_broll_matcher.py〕
...
**旅遊 cinematic:teal & orange 調色 + match cut 接地點(Sam Kolder 流派)**  〔expert-consensus ·常識｜Editkin v4 structured command(調色曲線 + 逐 frame match cut 對齊)/ ffmpeg(curves/colorbalance)〕
...
**motovlog:三層音訊混音(路噪+口白+BGM)+ 砍死時間**  〔data-backed ·常識｜Editkin v4 structured command(多音軌 + 音量自動化)/ ffmpeg(amix + acompressor,Hao 已有 M99)〕
